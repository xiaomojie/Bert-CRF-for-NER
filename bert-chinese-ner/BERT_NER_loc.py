...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        return ["O", "B-LOC", "I-LOC", "X", "[CLS]", "[SEP]"]
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    output_layer = model.get_sequence_output()  # 获取encoder最后一个隐藏层的输出， token-level output
    ...
    with tf.variable_scope("loss"):
        ...
        logits = tf.reshape(logits, [-1, FLAGS.max_seq_length, 7])  # 输出的是[序列长度，标签维度7维]
        ...
        per_example_loss = -tf.reduce_sum(one_hot_labels * log_probs, axis=-1)  # 交叉熵损失函数，一个example的损失，包含好几个命名实体标签,[序列长度，损失]
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            # def metric_fn(label_ids, logits):
                ...
                precision = tf_metrics.precision(label_ids, predictions, 7, [2, 3], average="macro")
                recall = tf_metrics.recall(label_ids, predictions, 7, [2, 3], average="macro")
                f = tf_metrics.f1(label_ids, predictions, 7, [2, 3], average="macro")
                #
                return {
